<x-app-layout>
    @section('page_title', 'Tambah Barang Inventaris')
    ...
    <div class="max-w-[800px] mx-auto">
        ...
        <div class="bg-white rounded-[16px] shadow-sm border border-[#E5E7EB] overflow-hidden">
            <div class="p-6 border-b border-gray-100 bg-[#F8FAF5]/40">
                <h3 class="text-base font-bold text-gray-900">Form Aset / Barang Baru</h3>
                <p class="text-xs text-gray-500">Silakan isi formulir di bawah ini untuk mencatat barang inventaris milik RT.</p>
            </div>

            <form action="{{ route('inventaris.store') }}" method="POST" class="p-6 space-y-5">
                @csrf

                <!-- Nama Barang -->
                <div>
                    <label for="nama_barang" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Nama Barang</label>
                    <input type="text" name="nama_barang" id="nama_barang" value="{{ old('nama_barang') }}" placeholder="Contoh: Tenda RT, Kursi Plastik"
                ...
                <div>
                    ...
                    <input type="number" name="jumlah" id="jumlah" value="{{ old('jumlah') }}" placeholder="0" min="1"
                           class="w-full text-xs rounded-xl border-[#E5E7EB] focus:border-[#2F5D34] focus:ring-[#2F5D34] shadow-sm" required>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="lokasi" id="lokasi" value="{{ old('lokasi') }}" placeholder="Contoh: Gudang RT, Rumah Pak RT"
                           class="w-full text-xs rounded-xl border-[#E5E7EB] focus:border-[#2F5D34] focus:ring-[#2F5D34] shadow-sm" required>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="tahun_perolehan" id="tahun_perolehan" value="{{ old('tahun_perolehan', date('Y')) }}" min="1900" max="{{ date('Y') }}"
                           class="w-full text-xs rounded-xl border-[#E5E7EB] focus:border-[#2F5D34] focus:ring-[#2F5D34] shadow-sm" required>
                </div>
                ...
                <div>
                    ...
                    <textarea name="keterangan" id="keterangan" placeholder="Keterangan tambahan mengenai barang..." rows="3"
                              class="w-full text-xs rounded-xl border-[#E5E7EB] focus:border-[#2F5D34] focus:ring-[#2F5D34] shadow-sm">{{ old('keterangan') }}</textarea>
                </div>
