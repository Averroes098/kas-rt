<x-app-layout>
    @section('page_title', 'Tambah Transaksi Kas')
    ...
    <div class="max-w-[800px] mx-auto">
        ...
        <div class="bg-white rounded-[16px] shadow-sm border border-[#E5E7EB] overflow-hidden">
            <div class="p-6 border-b border-gray-100 bg-[#F8FAF5]/40">
                <h3 class="text-base font-bold text-gray-900">Form Catatan Transaksi Baru</h3>
                <p class="text-xs text-gray-500">Isi lengkap kolom untuk mencatat transaksi keuangan RT.</p>
            </div>
            ...
            <form action="{{ route('transaksi.store') }}" method="POST" class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <select name="jenis" id="jenis" 
                            class="w-full text-xs rounded-xl border-[#E5E7EB] focus:border-[#2F5D34] focus:ring-[#2F5D34] shadow-sm" required>
                        <option value="masuk" {{ old('jenis') == 'masuk' ? 'selected' : '' }}>Kas Masuk (Pemasukan)</option>
                        <option value="keluar" {{ old('jenis') == 'keluar' ? 'selected' : '' }}>Kas Keluar (Pengeluaran)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="keterangan" id="keterangan" value="{{ old('keterangan') }}" placeholder="Contoh: Iuran bulanan warga, pembayaran sampah"
                           class="w-full text-xs rounded-xl border-[#E5E7EB] focus:border-[#2F5D34] focus:ring-[#2F5D34] shadow-sm" required>
                </div>
                ...
                <div>
                    ...
                    <div class="relative rounded-xl shadow-sm">
                        ...
                        <input type="number" name="nominal" id="nominal" value="{{ old('nominal') }}" placeholder="0" min="0" step="any"
